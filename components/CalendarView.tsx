...
import { Appointment, DailyCapacity, Profile } from '@/lib/types'
...
export default function CalendarView({ profile }: { profile: Profile }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [apptRes, capRes, settingsRes] = await Promise.all([
      supabase
  ...
  // Build grid: 6 weeks containing the current month
  ...
  function getCapacity(dateStr: string) {
    const cap = capacities.find(c => c.date === dateStr)
    return cap ? cap.max_trucks : defaultMax
  }
  ...
  const selectedDateMax = selectedDate ? getCapacity(selectedDate) : defaultMax
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-7 gap-px bg-gray-200 rounded-xl overflow-hidden border border-gray-200">
          {days.map(day => {
            ...
            const maxTrucks = getCapacity(dateStr)
            const count = dayAppts.length
            const full = count >= maxTrucks
                ...
                {count > 0 && (
                  <div className="mt-1">
                    <span className={`
                      ...
                      ${full ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'}
                    `}>
                      {count}/{maxTrucks}
                    </span>
                  </div>
